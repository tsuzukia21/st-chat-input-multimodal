...
- 🎤 **音声入力**: Web Speech API / サーバー側 OpenAI Whisper 対応
- 🔒 **入力バリデーション**: ファイル数、ファイルサイズ、MIME/実データ、各種パラメータを検証
- 🎨 **Streamlit標準テーマ**: 完全対応デザイン
- 🔄 **Drag & Drop**: ファイルのドラッグ&ドロップ対応
- ⌨️ **Ctrl+V**: クリップボードからの画像貼り付け
- 🚨 **インラインエラー表示**: 安全なメッセージを表示
...
Python 側の仮想環境は `uv` が `.venv` で管理し、依存関係は `uv.lock` で固定します。
...
# OpenAI Whisper をサーバー側で使用する場合
# 推奨: Python 実行環境に OPENAI_API_KEY を設定
result = multimodal_chat_input(
    enable_voice_input=True,
...
`openai_whisper` を選んだ場合、録音データは Python バックエンドへ送られて文字起こしされます。API キーはサーバー側でのみ利用され、ブラウザには送信されません。
...
アップロード画像は拡張子とファイルシグネチャの両方で検証され、表示前にファイル名はサニタイズされます。
...
[streamlit-chatbot の例](https://github.com/tsuzukia21/streamlit-chatbot)
